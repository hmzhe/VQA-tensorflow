...
def prepro_question_answer(imgs):
  ...
  for i,img in enumerate(imgs):
    ...
    for j, qa_pair in enumerate(img['qa_pairs']):
      question_txt = str(qa_pair['question']).lower().translate(str.maketrans('','',string.punctuation)).strip().split()
      img['processed_question_tokens'].append(question_txt)
      answer_txt = str(qa_pair['answer']).lower().translate(str.maketrans('','',string.punctuation)).strip().split()
      img['processed_answer_tokens'].append(answer_txt)
      processed_mc_tokens = []
      if 'multiple_choices' in qa_pair:
        for mc in qa_pair['multiple_choices']:
          mc_txt = str(mc).lower().translate(str.maketrans('','',string.punctuation)).strip().split()
...
def encode_question_answer(imgs, params, wtoi):
  ...
  for i,img in enumerate(imgs):
    ...
    split_ = np.zeros((N,), dtype='uint32')
    if img['split'] == 'val':
      split_[i] = 1
    elif img['split'] == 'test':
      split_[i] = 2
  ...
  return Lq, La, Lmc, label_start_ix, label_end_ix, question_label_length, answer_label_length, label_id,split_
...
if __name__ == "__main__":
  ...
  parser.add_argument('--dataset_json', default='data/dataset_v7w_telling.json', help='input dataset json file')
  ...
  parser.add_argument('--image_path', default='data/images/v7w_%s.jpg', help='path template based on image id')
  ...
  Lq, La, Lmc, label_start_ix, label_end_ix, question_label_length, answer_label_length, label_id,split_ = encode_question_answer(dataset['images'], params, wtoi)
  ...
  f.create_dataset('split',dtype='uint32', data=split_)
